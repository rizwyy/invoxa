...
export const workspaceSchema = z.object({ name: z.string().trim().min(2).max(100), reminders: z.boolean(), reminderDays: z.number().int().min(1).max(30) })
export const uploadSchema = z.object({ name: z.string().trim().min(1).max(180).regex(/^[^/\\\x00-\x1f]+$/), type: z.enum(['application/pdf', 'image/jpeg', 'image/png']), size: z.number().int().positive().max(MAX_BYTES) })
export const today = (now = new Date()) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Kolkata', year: 'numeric', month: '2-digit', day: '2-digit' }).format(now)
export function plusDays(value: string, days: number) { const d = new Date(`${value}T00:00:00Z`); d.setUTCDate(d.getUTCDate() + days); return d.toISOString().slice(0, 10) }
...
export function summary(invoices: Invoice[], day = today()) {
  ...
  for (const i of monthly) vendors.set(i.vendor, (vendors.get(i.vendor) || 0) + i.total)
  return {
    outstanding: unpaid.reduce((n, i) => n + i.total, 0), overdue: unpaid.filter(i => i.due && i.due < day).reduce((n, i) => n + i.total, 0),
    dueSoon: unpaid.filter(i => i.due && i.due >= day && i.due <= plusDays(day, 7)).length,
    monthlyCount: monthly.length, monthlyTotal: monthly.reduce((n, i) => n + i.total, 0), monthlyTax: monthly.reduce((n, i) => n + i.tax, 0),
