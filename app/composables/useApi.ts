...
export function useApi() {
  ...
  return async <T>(
    path: string,
    options: { method?: "GET" | "POST" | "PATCH" | "PUT"; body?: any } = {},
  ): Promise<T> => {
    ...
    try {
      return (await $fetch<T>(
        `${config.appMode === "aws" ? config.apiBase : "/api"}${path}`,
        {
          ...options,
          headers,
          credentials: config.appMode === "aws" ? "omit" : "same-origin",
        },
      )) as T;
    } catch (e: any) {
      if (e.status === 401 && import.meta.client) {
        sessionStorage.removeItem("invoxa_access");
        account.value = null;
        invoices.value = [];
        if (
          !path.startsWith("/auth/") &&
          normalizeRoutePath(router.currentRoute.value.path) !== "/login"
        )
