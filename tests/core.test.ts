...
test('overdue boundaries, missing due dates, paid and unreviewed totals', () => {
  ...
  const s = summary([invoice(), invoice({ reviewed: false }), invoice({ archived: true }), invoice({ payment: 'paid' })], '2026-09-12')
  assert.equal(s.outstanding, 11800); assert.equal(s.overdue, 0); assert.equal(s.monthlyTotal, 23600)
})
test('tenant isolation covers reads, edits, file access and archive operations', async t => {
  const { service, files } = await fixture(t); const i = await uploaded(service, files)
  await service.createWorkspace(bob, { name: 'Bob business' })
  assert.equal((await service.list(bob)).length, 0)
  for (const fn of [() => service.get(bob, i.id), () => service.edit(bob, i.id, { ...fields(), version: i.version }), () => service.document(bob, i.id), () => service.action(bob, i.id, { action: 'archive', version: i.version })]) await assert.rejects(fn, /not found/)
...
test('uploads persist, originals freeze, review requires acknowledgement and stale writes conflict', async t => {
  const { service, files, dir } = await fixture(t); const i = await uploaded(service, files)
  assert.equal(i.extraction?.source, 'manual'); assert.equal(i.processing, 'needs-review')
  await assert.rejects(() => service.edit(alice, i.id, { ...fields(), acknowledged: false, version: i.version }), /reviewed/)
  await assert.rejects(() => service.edit(alice, i.id, { ...fields(), total: 11801, version: i.version }), /difference/)
  const saved = await service.edit(alice, i.id, { ...fields(), version: i.version })
  await assert.rejects(() => service.edit(alice, i.id, { ...fields(), version: i.version }), Conflict)
  const restarted = new InvoiceService(new LocalRepository(dir), new LocalFiles(dir))
  assert.equal((await restarted.get(alice, saved.id)).total, 11800)
  await assert.rejects(() => files.write(saved.file!.key, new Uint8Array([1, 2, 3])), /frozen/)
  const archived = await service.action(alice, saved.id, { action: 'archive', version: saved.version })
  assert.equal(summary(await service.list(alice)).outstanding, 0)
  const restored = await service.action(alice, saved.id, { action: 'restore', version: archived.version })
  const paid = await service.action(alice, saved.id, { action: 'paid', version: restored.version }); assert.ok(paid.paidAt)
})
test('file validation rejects spoofed, encrypted/invalid and oversized documents', async () => { await assert.rejects(() => validateFile(new TextEncoder().encode('<script>hi</script>'), 'image/png')); await assert.rejects(() => validateFile(new TextEncoder().encode('%PDF-invalid'), 'application/pdf')); const doc = await PDFDocument.create(); for (let n = 0; n < 11; n++) doc.addPage(); await assert.rejects(() => validateFile(awaited(), 'application/pdf')); function awaited() { return new Uint8Array(9 * 1024 * 1024) }; await assert.rejects(() => doc.save().then(b => validateFile(b, 'application/pdf'))); assert.equal((await validateFile(await pdf(), 'application/pdf')).length, 64) })
test('local passwords are hashed, sessions survive restart and logout invalidates sessions', async t => { const { repo, dir } = await fixture(t); const auth = new LocalAuth(repo); const token = await auth.signup({ email: 'owner@example.test', password: 'long-unique-test-password' }); assert.equal((await new LocalAuth(new LocalRepository(dir)).identify(token)).email, 'owner@example.test'); const stored = await repo.get('EMAIL#owner@example.test', 'ACCOUNT'); assert.equal(stored?.data.password, undefined); await assert.rejects(() => auth.login({ email: 'owner@example.test', password: 'a-wrong-password' }), /incorrect/); await auth.logout(token); await assert.rejects(() => auth.identify(token), /expired/) })
test('concurrent quota requests cannot exceed configured cap', async t => { const { service } = await fixture(t); const results = await Promise.allSettled(Array.from({ length: 8 }, () => service.consumeQuota('test', 'day', 3))); assert.equal(results.filter(r => r.status === 'fulfilled').length, 3) })
...
test('reminders respect opt-in and payment state and are claimed once per day', async t => {
  ...
  let sends = 0; const mailer = { async send() { sends++ } }
  await runReminders(repo, mailer, '2026-09-12'); assert.equal(sends, 0)
  await service.updateWorkspace(alice, { ...w, reminders: true }); await runReminders(repo, mailer, '2026-09-12'); await runReminders(repo, mailer, '2026-09-12'); assert.equal(sends, 1)
  assert.equal(reminderInvoices([invoice({ payment: 'paid' }), invoice({ due: '' }), invoice({ reviewed: false })], w, '2026-09-12').length, 0)
})
test('CSV export escapes formulas and quotes', () => { const text = csv([invoice({ vendor: '=1+2', number: 'a"b' })]); assert.ok(text.includes('"\'=1+2"')); assert.ok(text.includes('"a""b"')) })
test('rejected documents cannot be previewed or confirmed', async t => {
  const { service, files } = await fixture(t); await service.createWorkspace(alice, { name: 'Test workspace' })
  const bytes = new TextEncoder().encode('this is not a PDF')
  const { invoice: i } = await service.reserve(alice, { name: 'bad.pdf', type: 'application/pdf', size: bytes.length })
  ...
  await assert.rejects(() => service.edit(alice, i.id, { ...fields(), version: rejected.version }), /valid document/)
})
test('reminder storage failures are surfaced instead of silently skipped', async t => {
  const { service, files, repo } = await fixture(t); const i = await uploaded(service, files)
  await service.edit(alice, i.id, { ...fields(), version: i.version })
  const w = (await service.workspace(alice))!; await service.updateWorkspace(alice, { ...w, reminders: true })
  repo.put = async () => { throw new Error('Database unavailable') }
  await assert.rejects(() => runReminders(repo, { async send() { throw new Error('Must not send') } }, '2026-09-12'), /Database unavailable/)
})
